...
public class Vectlist {
    ...
    public Vectlist(){
        ...
        Arc ui = new Arc('B','A',-2,0, 'e');
        list[21] = ui;

        Arc v =  new Arc('B','C',5,0,'d');
        list[1] = v;

        Arc vi =  new Arc('C','B',-5,0,'e');
        list[22] = vi;

        Arc w = new Arc('C','D',2,0,'d');
        list[2] = w;

        Arc wi = new Arc('D','C',-2,0,'e');
        list[23] = wi;

        Arc z = new Arc('D','E',2,0,'d');
        list[3] = z;

        Arc zi = new Arc('E','D',-2,0,'b');
        list[24] = zi;

        Arc a = new Arc('A','F',0,-2,'b');
        list[4] = a;

        Arc ai = new Arc('F','A',0,2,'d');
        list[25] = ai;

        Arc b = new Arc('B','G',0,-2,'d');
        list[5] = b;

        Arc bi = new Arc('G','B',0,2,'e');
        list[26] = bi;

        Arc c = new Arc('C', 'H', 0, -2, 'd');
        list[6] = c;

        Arc ci = new Arc('H', 'C', 0, 2 ,'b');
        list[27] = ci;

        Arc d = new Arc('D','J',0,-4,'d');
        list[7] = d;

        Arc di = new Arc('J','D',0,4,'f');
        list[28] = di;
        Arc e = new Arc('E','K',0,-4,'a');
        list[8]=e;

        Arc ei = new Arc('K','E',0,4,'c');
        list[29]=ei;
